...
it('authenticates with the configured key and base url', function () {
    ...
    Http::assertSent(fn (Request $request): bool => $request->url() === 'https://send.noria.test/v1/emails'
...
it('sends through a stored template when the template header is present', function () {
    ...
    Mail::raw('ignored', function ($message) {
        $message->to('a@example.test')->subject('s');
        $message->getHeaders()->addTextHeader(SendTransport::TEMPLATE_HEADER, 'welcome');
        $message->getHeaders()->addTextHeader(SendTransport::VARIABLES_HEADER, json_encode(['name' => 'Gitonga']));
    });
...
it('passes an idempotency key as a header rather than in the body', function () {
    ...
    Mail::raw('body', function ($message) {
        $message->to('a@example.test')->subject('s');
        $message->getHeaders()->addTextHeader(SendTransport::IDEMPOTENCY_HEADER, 'signin-42');
    });
...
it('still raises other API errors', function () {
    ...
    Mail::raw('body', fn ($message) => $message->to('a@example.test')->subject('s'));
})->throws(SendException::class, 'not verified');
...
it('retries a 503 and then succeeds', function () {
    ...
    $result = app(Send::class)->emails()->send(['from' => 'a@b.test', 'to' => 'c@d.test', 'subject' => 's', 'text' => 't']);
...
it('does not retry a quota rejection', function () {
    ...
    expect(fn () => app(Send::class)->emails()->send(['from' => 'a@b.test', 'to' => 'c@d.test', 'subject' => 's', 'text' => 't']))
        ->toThrow(SendException::class);
...
it('exposes the rest of the API surface', function () {
    ...
    $send->sms()->send(['from' => 'NORIA', 'to' => '0712000001', 'text' => 'hi']);
    $send->sms()->sendBatch([['to' => '0712000002', 'text' => 'hi']]);
    $send->domains()->create('example.test');
    $send->senders()->create('NORIA');
    $send->templates()->upsert(['slug' => 'welcome', 'subject' => 's', 'text' => 't']);
    $send->suppressions()->add('0712000003', 'sms', 'unsubscribe');
    $send->webhooks()->create('https://hook.test', ['delivered']);
    $send->messages()->requeue('msg_1');
    $send->messages()->events('msg_1');
...
it('reports whether a destination is suppressed', function () {
    ...
    expect(app(Send::class)->suppressions()->has('a@b.test'))->toBeTrue();
});
...
it('rejects a tampered webhook body, a stale timestamp and a malformed header', function () {
    ...
    expect(fn () => $verifier->verify(str_replace('delivered', 'bounced', $payload), $signature))
        ->toThrow(SendException::class, 'Invalid webhook signature');
    ...
    expect(fn () => $verifier->verify($payload, $staleSignature))
        ->toThrow(SendException::class, 'outside the tolerance window');

    expect(fn () => $verifier->verify($payload, 'nonsense'))
        ->toThrow(SendException::class, 'Malformed Noria-Signature header');
});

it('talks to production unless told otherwise', function () {
    Http::fake(['*' => Http::response(['object' => 'list', 'data' => []], 200)]);

    (new Send(app(Factory::class), 'nm_live_x'))->messages()->list();

    Http::assertSent(fn (Request $request): bool => $request->url() === 'https://send.noria.co.ke/v1/messages');
});

it('requires an api key', function () {
    new Send(app(Factory::class), '');
})->throws(SendException::class, 'API key is required');
...
class OtpIssued extends Notification
{
    ...
    public function toSendSms(mixed $notifiable): SmsMessage
    {
...
class PlainOtp extends Notification
{
    ...
    public function toSendSms(mixed $notifiable): string
    {
...
it('sends an sms notification through the send-sms channel', function () {
    Http::fake(['*' => Http::response(['id' => 'msg_01sms', 'object' => 'sms', 'status' => 'queued'], 202)]);

    NotificationFacade::send([new Subscriber], new OtpIssued);

    Http::assertSent(fn (Request $request): bool => $request->url() === 'https://send.noria.test/v1/sms');
